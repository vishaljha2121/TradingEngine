...
function SlipTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-[#0B1220] border border-[#1F2A3A] rounded shadow-xl font-ui overflow-hidden min-w-[140px]">
      ...
      <div className="px-3 py-2 flex items-center justify-between relative">
        <span className="text-[#6F7C8E] text-[10px] font-mono">{label}</span>
        <span className="text-[#E5EDF7] font-mono font-bold text-[12px] tabular-nums">{Math.abs(slipVal).toFixed(2)} bps</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-3 h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-[#0B1220] border border-[#1F2A3A]/70 rounded-xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-[#1F2A3A]/40 flex items-center justify-between flex-shrink-0 bg-[#0E1728]/30">
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
              <span className="flex items-center gap-1.5 text-[10px] text-[#A8B3C2] font-ui uppercase font-semibold">
                <div className="w-2.5 h-2.5 rounded-full bg-[#18C37E]/40 border border-[#18C37E]" /> Tight Coverage
              </span>
              <span className="flex items-center gap-1.5 text-[10px] text-[#A8B3C2] font-ui uppercase font-semibold ml-2">
                <div className="w-2.5 h-2.5 rounded-full bg-[#FF5C5C]/40 border border-[#FF5C5C]" /> Wider Pricing
              </span>
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-[11px] uppercase font-ui font-semibold text-[#6F7C8E]">Current Delta</span>
            <span className={`text-[16px] font-mono font-black tabular-nums ${spread <= 0 ? 'text-[#18C37E]' : 'text-[#FF5C5C]'}`}>
              {spread > 0 ? '+' : ''}{spread.toFixed(2)} bps
            </span>
          </div>
        </div>

        <div className="flex-1 min-h-[50px] px-3 pt-4 pb-2 relative">
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-[#0B1220] border border-[#1F2A3A]/70 rounded-xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-[#1F2A3A]/40 flex items-center justify-between flex-shrink-0 bg-[#0E1728]/30">
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
              <span className="flex items-center gap-1.5 text-[10px] text-[#A8B3C2] font-ui uppercase font-semibold">
                <div className="w-2.5 h-2.5 rounded-full bg-[#18C37E]/40 border border-[#18C37E]" /> Execution Savings
              </span>
              <span className="flex items-center gap-1.5 text-[10px] text-[#A8B3C2] font-ui uppercase font-semibold ml-2">
                <div className="w-2.5 h-2.5 rounded-full bg-[#FF5C5C]/40 border border-[#FF5C5C]" /> Cost Penalty
              </span>
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-[11px] uppercase font-ui font-semibold text-[#6F7C8E]">{slip >= 0 ? 'Current Savings' : 'Current Penalty'}</span>
            <span className={`text-[16px] font-mono font-black tabular-nums ${slip >= 0 ? 'text-[#18C37E]' : 'text-[#FF5C5C]'}`}>
              {Math.abs(slip).toFixed(2)} bps
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-[50px] px-3 pt-4 pb-2 relative">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={history} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              ...
              <CartesianGrid horizontal={true} vertical={false} stroke="#1F2A3A" strokeDasharray="4 4" opacity={0.4} />
              <XAxis dataKey="time" tick={{ fontSize: 10, fill: '#6F7C8E', fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={{ stroke: '#1F2A3A', strokeWidth: 1 }} interval="preserveEnd" minTickGap={80} />
              <YAxis domain={['dataMin - 0.5', 'dataMax + 0.5']} tick={{ fontSize: 10, fill: '#A8B3C2', fontFamily: 'JetBrains Mono' }} tickLine={false} axisLine={false} width={40} tickFormatter={(val) => Math.abs(val).toFixed(1)} />
              <ReferenceLine y={0} stroke="#E5EDF7" strokeWidth={1} strokeDasharray="6 6" opacity={0.5} label={{ value: 'Parity Baseline', position: 'insideTopLeft', fill: '#8EA0B8', fontSize: 10, fontFamily: 'ui-sans-serif', dy: -12 }} />
