...
function Arrow({ ok }: { ok: boolean }) {
  return ok
    ? <span className="text-[#18C37E] text-[11px] ml-2">▲</span>
    : <span className="text-[#FF5C5C] text-[11px] ml-2">▼</span>;
}
...
function InCellBar({ tm, bench, ok }: { tm: number, bench: number, ok: boolean }) {
  ...
  return (
    <div className="w-[84px] h-[5px] bg-[#1F2A3A] rounded-full overflow-hidden flex mx-auto mt-1.5 shadow-inner">
...
export function VenueComparisonTable({ truemarkets, benchmark, lagMs, benchName }: VenueComparisonTableProps) {
  ...
  const rows = [
    {
      metric: 'Absolute Slippage Cost',
      tooltip: { title: 'Net Execution Cost', description: 'Execution execution premium or penalty against the benchmark volume.' },
      tm: `${Math.abs(slipAdv).toFixed(2)} bps`,
      bench: 'Parity Baseline',
      delta: `${Math.abs(slipAdv).toFixed(2)} bps`,
      ok: slipAdv >= 0,
      verdict: slipAdv >= 0 ? 'Execution Savings' : 'Cost Penalty',
      bar: null,
      urgent: true
    },
    {
      ...
      ok: spreadGap <= 0,
      verdict: spreadGap <= 0 ? 'Tight Pricing' : 'Wider Spreads',
      bar: null,
      urgent: true
    },
    {
      ...
      delta: `+${lagMs} ms`,
      ok: lagMs <= 100,
      verdict: lagMs <= 50 ? 'Nominal Reaction' : lagMs <= 100 ? 'Caution' : 'Severe Delay',
      bar: null,
      urgent: false
    },
    {
  ...
  return (
    <div className="bg-[#0B1220] border border-[#1F2A3A]/70 rounded-xl flex flex-col overflow-hidden h-full shadow-sm">
      <div className="px-7 py-5 border-b border-[#1F2A3A]/40 flex items-center justify-between flex-shrink-0 bg-[#0E1728]/20">
        <span className="text-[15px] font-bold tracking-widest text-[#E5EDF7] font-ui uppercase">Liquidity Profile Engine</span>
        <div className="flex gap-4">
           <div className="flex items-center gap-2.5 bg-[#0B1220] px-4 py-1.5 rounded border border-[#1F2A3A]/50 shadow-inner">
             <span className="text-[12px] text-[#A8B3C2] uppercase tracking-wider font-ui font-semibold">Overall Depth Ratio</span>
             <span className={`font-mono font-bold text-[14px] tabular-nums ${ratioColor}`}>{totalRatio.toFixed(2)}×</span>
           </div>
        </div>
      </div>

      <div className="grid grid-cols-[1.6fr_1fr_1fr_1fr_1fr] px-8 py-4 text-[13px] uppercase tracking-widest font-bold text-[#6F7C8E] font-ui border-b border-[#1F2A3A]/30 flex-shrink-0 bg-[#0A101C]">
      ...
      <div className="flex-1 flex flex-col py-2 overflow-auto">
        {rows.map((row, i) => (
          <div
            key={i}
            className={`grid grid-cols-[1.6fr_1fr_1fr_1fr_1fr] px-8 py-5 text-[15px] items-center font-mono border-b border-[#1F2A3A]/10 last:border-b-0 group transition-colors ${row.urgent ? 'bg-[#0E1728]/10 hover:bg-[#0E1728]/30' : 'hover:bg-[#0E1728]/30'}`}
          >
            <div className="font-ui font-bold text-[#8EA0B8] text-[14px] uppercase tracking-wide group-hover:text-[#E5EDF7] transition-colors">
            ...
            <div className="flex flex-col text-center">
              <span className={`tabular-nums ${row.urgent ? 'text-[#E5EDF7] font-bold text-[16px]' : 'text-[#A8B3C2] text-[15px]'}`}>{row.tm}</span>
              {row.bar}
            </div>
            
            <div className="text-center text-[#6F7C8E] tabular-nums text-[15px]">{row.bench}</div>
            
            <div className={`text-center font-bold tabular-nums text-[15px] ${row.ok ? 'text-[#18C37E]/90' : 'text-[#FF5C5C]/90'}`}>
              {row.delta}
            </div>

            <div className="text-right flex items-center justify-end">
              <span className={`text-[14px] font-black font-ui uppercase tracking-widest ${row.ok ? 'text-[#18C37E]' : 'text-[#FF5C5C]'}`}>
                {row.verdict}
              </span>
              <Arrow ok={row.ok} />
            </div>
